...
async function seedDatabase() {
    try {
        ...
        console.log("🗑️ Очищены коллекции Product и Category")
        ...
        const products: Omit<IProduct, "_id">[] = jsonProducts.map((item) => {
            ...
            const prices: Price[] = item.prices?.map(({size, price}) => ({
                size, price
            })) || []

            const categoriesIds = item.categories
                ?.map(category => categoryDocs.get(category)) as Types.ObjectId[] || []
