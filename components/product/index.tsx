    ...
    return (
        ...
        >
            ...
            <Flex direction="column" flex="1" p={{ base: 5, md: 6 }}>
                ...
                {/* Цены и кнопка — без изменений */}
                <Flex direction="column" mt="auto" gap={3} minH="100px">
                    <AnimatePresence mode="popLayout">
                        {selecting ? (
                            <motion.div key="selecting" layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                                <Flex direction="column" gap={2}>
                                    {prices?.map((p) => (
                                        <Button
                                            ...
                                            borderWidth="1px"
                                            borderColor="teal.500"
                                            _hover={{ bg: selectedPrice?.size === p.size ? "teal.600" : "gray.700", color: "white" }}
                                    ...
                                    <Flex justify="space-between" mt={2}>
                                        <Button size="sm" borderRadius="full" bg="red.500" color="white" _hover={{ bg: "red.600" }} onClick={handleCancel}>
                                            Отменить
                                        </Button>
                                        <Button size="sm" borderRadius="full" bg="teal.500" color="white" _hover={{ bg: "teal.600" }} onClick={handleConfirm}>
                                            Подтвердить
                                        </Button>
                                    </Flex>
                                </Flex>
                            ...
                            <motion.div key="prices" layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                                <Flex direction="column" gap={2}>
                    ...
                    {!selecting && (
                        <MotionButton
                            ...
                            fontWeight="medium"
                            whileTap={{ scale: 0.95 }}
                            animate={added ? { scale: [1, 1.05, 1], boxShadow: ["0 0 0 rgba(56,178,172,0)", "0 0 20px rgba(56,178,172,0.6)", "0 0 0 rgba(56,178,172,0)"] } : {}}
                            _hover={{ bg: added ? "teal.600" : "teal.500", color: "white", boxShadow: "0 4px 12px rgba(56,178,172,0.35)" }}
                            onClick={handleAddClick}
                        >
                            <AnimatePresence mode="wait">
                                {added ? (
                                    <motion.div key="added" initial={{ opacity: 0, y: 10, scale: 0.8 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -10, scale: 0.9 }}>
