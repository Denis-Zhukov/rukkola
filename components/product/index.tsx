...
export const Product = ({id, img, alt, title, description, prices}: ProductProps) => {
    ...
    // Авто-выбор первого размера при открытии
    useEffect(() => {
    ...
    const handleAddClick = () => {
        ...
        if (prices.length > 1) {
            setSelecting(true); // useEffect выберет первый
        } else {
            // ОДИН РАЗМЕР: НЕ МЕНЯЕМ СОСТОЯНИЕ — просто добавляем
            addToCart({
                id,
                name: title,
                image: img ?? "",
